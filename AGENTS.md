...
Agents can spawn sibling top-level threads with the `spawn_thread` MCP tool, capped at five successful spawns per provider session. A spawn is fire-and-forget: the new thread appears in the sidebar like a user-created thread, starts immediately on the self-contained prompt it was given, and never reports back — there is no parent-child channel. The tool is discoverable by name and deliberately absent from the always-on tool-instruction blocks.

Provider instance and permission mode always inherit from the spawning thread and are never arguments, so an agent cannot aim a spawn at another account or a more permissive mode than its own. The agent may name a `model`, and that is deliberately not a runtime choice: it selects a model **within the inherited provider instance**, which is what "a different runtime is the user's choice, not the agent's" means and does not restrict. Letting the agent choose keeps a costly parent from minting equally costly children for intern-shaped work. The interaction mode is always default even from a plan-mode parent, because a delegation is given work to do, not a plan to propose.
...
Route every spawn through the shared thread-bootstrap runner rather than a second copy of the create-and-start logic, so a spawned thread is indistinguishable from one the user started by hand.
