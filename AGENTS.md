...
Agents can spawn sibling top-level threads with the `spawn_thread` MCP tool, capped at five successful spawns per provider session. A spawn is fire-and-forget: the new thread appears in the sidebar like a user-created thread, starts immediately on the self-contained prompt it was given, and never reports back — there is no parent-child channel. Provider instance and permission mode always inherit from the spawning thread and are never arguments, and the interaction mode is always default even from a plan-mode parent. The project is derived from the optional directory argument, never chosen: a path that is exactly a known project's workspace root files the child under that project with no worktree override, any other existing absolute directory rides as a worktree of the parent's project, and no project is ever created for an unmatched path. The tool is discoverable by name and deliberately absent from the always-on tool-instruction blocks.
