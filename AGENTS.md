...
- Support the providers Chris uses—OpenAI Codex, Anthropic Claude Code, and OpenCode (which fronts local runtimes such as LM Studio and aggregators such as OpenRouter)—and do not carry Cursor, Grok, or their implementation-only supporting paths. Legacy provider identifiers may remain only at persistence and wire boundaries where removing them would break existing settings or released clients.
...
An agent can delegate work to another thread with the `spawn_thread` MCP tool, capped at five successful spawns per provider session. The caller declares which of two delegations it means, per call, because the same agent legitimately wants both. A **hand-off** — the default, and the weaker grant — is started on the user's behalf: a sibling top-level thread that appears in the sidebar like a user-created one, starts immediately on the self-contained prompt it was given, and has no channel back to the caller in either direction. A **teammate** belongs to the caller: parentage is recorded, the sidebar nests it under its parent, and `message_thread` carries results, answers, and blockers up and further context down. Only a teammate is told it was delegated to and how to reply, because a hand-off has no reply channel and saying otherwise would produce a refusal it cannot act on.

Provider instance and permission mode always inherit from the spawning thread and are never arguments, so an agent cannot aim a spawn at another account or a more permissive mode than its own. The agent may name a `model`, and that is deliberately not a runtime choice: it selects a model **within the inherited provider instance**, which is what "a different runtime is the user's choice, not the agent's" means and does not restrict. Letting the agent choose keeps a costly parent from minting equally costly children for intern-shaped work. The interaction mode does not inherit — a delegation is given work to do, not a plan to propose, so it defaults to default even from a plan-mode parent — but the parent may override it explicitly when it wants the approach before the action. An agent profile is the opposite of an inheritance: a caller running under a profile spawns a profile-less child unless the spawn names one explicitly with `agentProfile`, because a profile carries instructions and a tool policy that were chosen for the caller's job, not the child's. Naming a profile does not change the child's model either—the child keeps the caller's model, or the `model` the caller named—so a profile's own model preference is inert here, and `model` remains the only way to move a child onto a different model.

Where the new thread works is the caller's choice between two mutually exclusive arguments, and passing both is refused rather than silently resolved. `directory` takes a checkout as it stands, and the project is derived from it, never chosen: a path that is exactly a known project's workspace root files the child under that project with no worktree override, any other existing absolute directory rides as a worktree of the parent's project, and no project is ever created for an unmatched path. `repositoryPath` instead cuts a fresh worktree on a new temporary branch, which is what delegated work that edits files needs, since two agents in one checkout contend. The project never owns the other repository, only the worktree and branch cut from it, so there is no multi-root project and each thread keeps exactly one repository for checkpointing. Never remove a spawned worktree or branch automatically, and never let an agent tear one down — it may hold uncommitted work.

Route every spawn through the shared thread-bootstrap runner rather than a second copy of the create-and-start logic, so a spawned thread is indistinguishable from one the user started by hand. `message_thread` reaches only the caller's parent or a teammate it spawned; parentage is the entire authorization model, and it is read from the credential's thread and the recipient's own row, never from an argument. Sibling and child-to-child messaging is out of scope: this checks a single parent pointer in each direction, and a team would need a persisted team entity. `threadId` stays optional and an omitted recipient means the caller's parent, resolved server-side — a spawned agent is never told its parent's id, so requiring one makes replying upward impossible and inviting a guess misdelivers. Deliver a message as the recipient's next queued turn, never injected mid-turn: Claude's native peer messaging is observed by the adapter rather than routed by CPH and has no Codex equivalent, so a queued turn is what keeps this provider-neutral.

Both tools are discoverable by name and deliberately absent from the always-on tool-instruction blocks. The accepted consequence is that an agent asked to "spin up an agent in the Y repo" will, absent instruction, reach for its own harness subagent — which runs in the wrong repository, inside the caller's session, and leaves no thread the user can open. That is the user's to correct by naming the tool, and it is a deliberate choice rather than a gap to fix.
...
- **provider** means the agent runtime or harness CPH Code talks to: Codex, Claude, or OpenCode.
...
- **Providers.** Codex, Claude, and OpenCode each have an adapter. Provider-shaped features need a decision for both, even when one remains explicitly unimplemented pending a real provider payload.
