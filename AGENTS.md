...
Provider instance and permission mode always inherit from the spawning thread and are never arguments, so an agent cannot aim a spawn at another account or a more permissive mode than its own. The agent may name a `model`, and that is deliberately not a runtime choice: it selects a model **within the inherited provider instance**, which is what "a different runtime is the user's choice, not the agent's" means and does not restrict. Letting the agent choose keeps a costly parent from minting equally costly children for intern-shaped work. The interaction mode does not inherit — a delegation is given work to do, not a plan to propose, so it defaults to default even from a plan-mode parent — but the parent may override it explicitly when it wants the approach before the action.
