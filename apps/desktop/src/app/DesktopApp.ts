...
const startup = Effect.gen(function* () {
  ...
  if (shellEnvironmentFiber !== null) {
    yield* Fiber.join(shellEnvironmentFiber);
  }
  ...
  // The backend is the long pole for first window. Start it before refreshing
  // post-ready app chrome so its cold boot overlaps Dock icon/menu setup. The
  // window still cannot open until bootstrap has registered the protocol and
  // IPC handlers, and backend readiness normally arrives much later.
