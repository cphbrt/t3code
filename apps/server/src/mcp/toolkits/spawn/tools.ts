...
export class SpawnThreadError extends Schema.TaggedErrorClass<SpawnThreadError>()(
  "SpawnThreadError",
  {
    ...
    // invalid-argument: prompt or title was empty, or model was blank.
...
export const SpawnThreadTool = Tool.make("spawn_thread", {
  description: `Spawn a new top-level thread in the sidebar, as if the user had opened it themselves. It immediately starts working on the prompt you pass, with this thread's permission mode, on its own fresh provider session. It is fire-and-forget: it does not report back to you, and this tool gives you no way to message it — the user follows its work in the sidebar like any other thread. Use it to hand off an independent workstream that should outlive this conversation; for subtasks whose results you need in this conversation, use your own subagent tools instead. The new thread starts with none of your context, so the prompt must be self-contained. At most ${SPAWN_LIMIT_PER_SESSION} spawns per session.`,
  parameters: Schema.Struct({
    ...
    directory: Schema.optional(
      Schema.String.annotate({
        description:
          "Absolute path to an existing directory the new thread works in. Omit to use this thread's own working directory. If the path is the root of a project the user already has, the new thread files under that project; otherwise it stays in this thread's project.",
      }),
    ),
    model: Schema.optional(
      Schema.String.annotate({
        description:
          "Model id for the new thread, on this thread's provider. Omit to use this thread's model.",
      }),
    ),
  }),
