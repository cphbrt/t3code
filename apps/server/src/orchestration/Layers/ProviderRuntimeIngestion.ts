...
const make = Effect.gen(function* () {
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      // steering a running turn can make a provider open a new turn without
      ...
      if (
        ...
        const status = (() => {
          switch (event.type) {
            ...
            case "turn.started":
              return "running";
            case "session.exited":
              return activeTurnId !== null || hasPendingTurnStart ? "interrupted" : "stopped";
            case "turn.completed":
        ...
        if (shouldApplyThreadLifecycle) {
          ...
          if (event.type === "session.exited" && status === "interrupted") {
            yield* interruptThreadRuntime({
              threadId: thread.id,
