...
import { ModelSelection, ScheduledThreadTurn } from "@t3tools/contracts";

const ProjectionThreadDbRow = ProjectionThread.mapFields(
  Struct.assign({
    modelSelection: Schema.fromJsonString(ModelSelection),
    scheduledTurn: Schema.NullOr(Schema.fromJsonString(ScheduledThreadTurn)),
  }),
);
    ...
    execute: (row) =>
      sql`
        ...
        VALUES (
          ...
          ${row.scheduledTurn == null ? null : JSON.stringify(row.scheduledTurn)},
          ${row.latestUserMessageAt},
          ${row.pendingApprovalCount},
          ${row.pendingUserInputCount},
          ${row.hasActionableProposedPlan},
          ${row.unreadArtifactCount},
          ${row.deletedAt}
        )
        ...
        DO UPDATE SET
          ...
          scheduled_turn_json = excluded.scheduled_turn_json,
          latest_user_message_at = excluded.latest_user_message_at,
          pending_approval_count = excluded.pending_approval_count,
          pending_user_input_count = excluded.pending_user_input_count,
          has_actionable_proposed_plan = excluded.has_actionable_proposed_plan,
          unread_artifact_count = excluded.unread_artifact_count,
    ...
    execute: ({ threadId }) =>
      sql`
        SELECT
          ...
          scheduled_turn_json AS "scheduledTurn",
