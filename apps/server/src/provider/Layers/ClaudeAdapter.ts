...
function normalizeClaudeContextUsageApiSnapshot(
  ...
  const apiUsage = value.apiUsage;
  return makeClaudeTokenUsageSnapshot({
    activeTokens: value.totalTokens,
    contextWindow: value.maxTokens,
    ...(apiUsage
      ? {
          inputTokens: claudeUsageInputTokens(apiUsage),
          cachedInputTokens: apiUsage.cache_read_input_tokens,
          cacheWriteInputTokens: apiUsage.cache_creation_input_tokens,
        }
      : {}),
    ...(totalProcessedTokens !== undefined ? { totalProcessedTokens } : {}),
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      case "task_started": {
        ...
        // An assistant snapshot that beat this task_started buffered the
        // authoritative API model id. It outranks the launch override, but not
        // a model already refined on an existing record across a restart.
        const bufferedModel = message.tool_use_id
          ? context.pendingTaskModels.get(message.tool_use_id)
          : undefined;
        if (message.tool_use_id) {
          context.pendingTaskModels.delete(message.tool_use_id);
        }
