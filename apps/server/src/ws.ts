...
import { getSubagentTranscriptForThread } from "./orchestration/subagentTranscriptQuery.ts";
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const threadBootstrapRunner = yield* ThreadBootstrapRunner.ThreadBootstrapRunner;
      ...
      const dispatchNormalizedCommand = (
        ...
        const dispatchEffect =
          normalizedCommand.type === "thread.turn.start" && normalizedCommand.bootstrap
            ? threadBootstrapRunner.dispatchBootstrapTurnStart(normalizedCommand)
            : orchestrationEngine
      ...
      return WsRpcGroup.of({
        ...
        [ORCHESTRATION_WS_METHODS.getSubagentTranscript]: (input) =>
          observeRpcEffect(
            ...
            getSubagentTranscriptForThread({ threadId: input.threadId, agentId: input.agentId }),
            { "rpc.aggregate": "orchestration" },
          ),
    ...
    return HttpRouter.add(
      ...
      Effect.gen(function* () {
        ...
        const rpcWebSocketHttpEffect = yield* RpcServer.toHttpEffectWebsocket(WsRpcGroup, {
          ...
          Effect.provide(
            makeWsRpcLayer(session, previewAutomationBroker, sql).pipe(
