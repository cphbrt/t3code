...
1. Install the GitHub CLI (version 2.81.0 or newer) on the machine running CPH Code:
   ```bash
   brew install gh
   ```
2. Sign in:
...
- **GitHub says it could not verify sign-in status** – CPH Code needs GitHub CLI 2.81.0 or newer to check sign-in status. Update `gh` (e.g., `brew upgrade gh`), then rescan
